...
tilt_B = helixside.tilt_angle(caB_1afo, k_1afo, mask=sse_maskB_1afo)
...
for j in range(1, 5+1):
    ...
    # tilt of monomer
    tilt_M = helixside.tilt_angle(ca, k_M, mask=sse_mask)
    
    
    # difference between dimer and monomer
    diff_tilt = helixside.angle_diff(tilt_M,tilt_B)
    diff_tilt_angles.append(diff_tilt)
    
    #label for figure legend
    tilt_mean = helixside.circular_mean(diff_tilt)*180/np.pi
    tilt_mean = '{0:.1f}'.format( tilt_mean )
    tilt_std  = helixside.circular_std(diff_tilt) *180/np.pi
...
tilt_mean = helixside.circular_mean(diff_tilt_angles)*180/np.pi
tilt_mean  ='{0:.1f}'.format(tilt_mean)



tilt_std  = helixside.circular_std(diff_tilt_angles)*180/np.pi
tilt_std  ='{0:.1f}'.format(tilt_std)



fig, ax = helixside.plot.angle_density(diff_tilt_angles, clim=[-30,90], plot_many=True, bins=20,figsize=(15,7))
